...
async function loadContent() {
    ...
    try {
        const statuses = currentTab === 'pending' ? ['pending', 'pending_update'] : ['published'];
        const q = query(collection(db, "submissions"), where("status", "in", statuses));
        ...
        items.forEach(data => {
            ...
            const badgeText = isUpdate ? '✏️ Изменение' : (currentTab === 'pending' ? '➕ Новый' : '✅ Опубликован');
            ...
            if (currentTab === 'pending') {
                ...
                // Кнопка удаления теперь передает имя для подтверждения
                actionButtons = `
                    <button class="btn-edit" onclick="alert('Функция редактирования в разработке')">✏️ Ред.</button>
            ...
            card.innerHTML = `
                ...
                <p><strong>Статус:</strong> ${isUpdate ? 'Запрос на обновление' : (currentTab === 'pending' ? 'Новая запись' : 'Опубликован')}</p>
...
window.deleteHero = async (id, name) => {
    ...
    try {
        ...
        const targetData = subDoc.data();

        // 2. Находим и удаляем из коллекции heroes
        const heroesQ = query(collection(db, "heroes"), where("status", "==", "published"));
        const heroesSnap = await getDocs(heroesQ);
...
window.openEditModal = (id) => {
    alert('Функция редактирования админом в разработке.');
};
